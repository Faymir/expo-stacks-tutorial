...
export const useNavigationStore = create<NavigationState>((set) => ({
    ...
    set((state) => {
      if (state.stack.length === 0) {
        return { 
            ...
            lastAction: { type: 'replace', description: `Replaced empty stack with "${route}".`, timestamp: Date.now() }
        };
      }
  ...
  reset: (route) => {
      set({ 
          ...
          lastAction: { type: 'reset', description: `Reset stack to "${route}".`, timestamp: Date.now() }
      });
  }
}));
