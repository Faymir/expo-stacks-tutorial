...
const FileItem = ({ node, depth, activeRoute }: { node: FileNode; depth: number; activeRoute?: string }) => {
  ...
  const isActive = node.route && activeRoute ? activeRoute.includes(node.route.replace(/:[a-zA-Z]+/g, '')) : false;
  ...
  return (
    <div className="select-none">
      ...
      >
        ...
        <span className="mr-2">
          {isFolder ? <Folder size={16} className="text-yellow-500/80" /> : <FileCode size={16} className="text-blue-400/80" />}
        </span>
        
        <span className={clsx("text-sm", isActive && "font-medium text-blue-200")}>{node.name}</span>
        ...
        {isFolder && isOpen && node.children && (
          ...
          >
            {node.children.map((child, index) => (
              <FileItem key={index} node={child} depth={depth + 1} activeRoute={activeRoute} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
...
export const FileTree: React.FC<FileTreeProps> = ({ data, activeRoute }) => {
  return (
    <div className="w-80 bg-black/20 backdrop-blur-xl border border-white/10 rounded-xl p-4 flex flex-col flex-1 min-h-0">
      <h2 className="text-sm font-semibold text-gray-400 mb-4 uppercase tracking-wider">Project Structure</h2>
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        {data.map((node, index) => (
          <FileItem key={index} node={node} depth={0} activeRoute={activeRoute} />
        ))}
      </div>
    </div>
  );
