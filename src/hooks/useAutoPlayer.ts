...
import { type Scenario, TUTORIAL_SCENARIOS } from "../logic/tutorialScenarios";
...
export const useAutoPlayer = () => {
		...
		const timer = setTimeout(() => {
			switch (step.action) {
				case "navigate":
					...
					break;
				case "push":
					push(step.route, step.params);
					break;
				case "replace":
					replace(step.route, step.params);
					break;
				case "dismissTo":
					dismissTo(step.route);
					break;
				case "reset":
					reset(step.route);
					break;
	...
	return {
		...
		progress: currentScenario
			? (currentStepIndex / currentScenario.steps.length) * 100
			: 0,
	};
};
